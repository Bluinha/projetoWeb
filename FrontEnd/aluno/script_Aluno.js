...
function carregarNomesVacasDatalist() {
  ...
  fetch("../../BackEnd/get_vacas_json_com_id.php") // Novo endpoint PHP que retorna ID e Nome
    .then((response) => {
      if (!response.ok) {
        throw new Error(
          "Erro ao carregar os nomes das vacas: " + response.statusText
        );
      }
...
// Converte data BR (DD/MM/YYYY) para formato ISO (YYYY-MM-DD) para <input type="date">
...
function editarProducao(button) {
  ...
  dataCell.innerHTML = `<input type="date" class="input-edicao" value="${formatarParaInputDate(
    dataAtual
  )}">`;
...
function salvarEdicaoProducao(id, novaQtd, novaData, row, button) {
  ...
  fetch("../../BackEnd/producao/atualizar_producao_leite.php", {
    method: "POST",
    headers: {
      "Content-Type": "application/x-www-form-urlencoded",
    },
    body: `id_producao=${id}&id_vaca=${idVaca}&quantidade=${encodeURIComponent(
      novaQtd
    )}&data=${encodeURIComponent(novaData)}`,
  })
    .then((res) => res.json())
    .then((data) => {
      if (data.success) {
        row.querySelector(".quantidade-producao").textContent = novaQtd + " L";
        // Aqui convertemos para o formato DD/MM/YYYY
        row.querySelector(".data-producao").textContent =
          formatarDataBR(novaData);
        button.textContent = "Editar";
        button.classList.remove("btn-salvar");
        button.classList.add("btn-editar");
        button.onclick = function () {
...
function excluirProducao(id, button) {
  if (confirm("Tem certeza que deseja excluir essa produção?")) {
    ...
    if (!row) {
      alert(
        "Erro: não foi possível encontrar a linha da produção para excluir."
      );
      return;
    }
...
function editarTeste(button) {
  ...
  const resultadoAtual =
    resultadoCell.textContent.trim() === "Positivo" ? 1 : 0;
  ...
  resultadoCell.innerHTML = `
        <select class="input-edicao">
            <option value="1" ${
              resultadoAtual == 1 ? "selected" : ""
            }>Positivo</option>
            <option value="0" ${
              resultadoAtual == 0 ? "selected" : ""
            }>Negativo</option>
        </select>`;
  ...
  ubersCell.innerHTML = todasUbers
    .map((ubere) => {
...
function salvarEdicaoTeste(
  ...
  fetch("../../BackEnd/atualizar_teste_mastite.php", {
    ...
    body: `id_teste=${id}&resultado=${resultado}&quantas_cruzes=${cruzes}&tratamento=${encodeURIComponent(
      tratamento
    )}&observacoes=${encodeURIComponent(
      observacoes
    )}&ubere=${encodeURIComponent(ubereFormatado)}`,
  })
    .then((res) => res.json())
    .then((data) => {
      ...
      if (data.success) {
        ...
        row.querySelector(".resultado-teste").textContent =
          resultado == 1 ? "Positivo" : "Negativo";
